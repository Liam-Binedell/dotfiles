...
function M.set_scroll_dir(dir)
	...
	if workspace.tiled_layout == "scrolling" then
		if workspace.special then
			hl.workspace_rule({ workspace = workspace.name, layout_opts = { direction = dir } })
		else
			hl.workspace_rule({ workspace = tostring(workspace.id), layout_opts = { direction = dir } })
		end
	end
end
